...
> :warning: **.env передаётся отдельно (личо/секретно).** Это правило хорошего тона — даже для тестовых проектов не держать secrets в репозитории. В реальном проекте .env генерируется через secrets CI/CD.

```bash
# 1. Клонировать репозиторий
git clone https://github.com/kydaxtaxtax/test_it_link.git
cd test_it_link

# 2. Создать файл .env (передать лично/отдельно)
cp .env.example .env  # или получить файл от разработчика
